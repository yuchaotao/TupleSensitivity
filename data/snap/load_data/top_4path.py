...
'''%threshold
sql2 = '''
with same_circle_edges AS (SELECT * from same_circle_edges WHERE source = '348')
select * FROM (
select
    e1.source as source, e1.circle as circle1, e2.circle as circle2, e3.circle as circle3, e4.circle as circle4, count(*) as cnt
from
...
WHERE
    True
    AND t.circle1 < t.circle2
    AND t.circle2 < t.circle3
    AND t.circle3 < t.circle4
ORDER BY cnt DESC
LIMIT 1
;
'''%threshold
...
from
    same_circle_edges e1,  same_circle_edges e2,  same_circle_edges e3, same_circle_edges e4
WHERE
    e1.tnode = e2.fnode AND e2.tnode = e3.fnode AND e3.tnode = e4.fnode
GROUP BY
    e1.source, e1.circle, e2.circle, e3.circle, e4.circle
HAVING count(*) > %s
) AS t
...
results.to_csv('../%s/top_4path.csv'%plat, index = False)
