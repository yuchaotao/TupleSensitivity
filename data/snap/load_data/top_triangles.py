...
sql = '''
select
    e1.source as source, e1.circle as circle1, e2.circle as circle2, e3.circle as circle3, count(*) as cnt
from
...
GROUP BY
    e1.source, e1.circle, e2.circle, e3.circle
HAVING count(*) > %s
ORDER BY cnt DESC
;
'''%threshold
...
results.to_csv('../%s/top_triangles.csv'%plat, index = False)
