...
sql = '''
select
    tri.source as source, tri.circle as circle_tri, e1.circle as circle1, e2.circle as circle2, e3.circle as circle3, count(*) as cnt
from
...
GROUP BY
    tri.source, e1.circle, e2.circle, e3.circle, tri.circle
HAVING count(*) > %s
ORDER BY cnt DESC
;
'''%threshold
